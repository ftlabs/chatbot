...
module.exports = function(res, key, indexPrefixStr) {
	...
	var brain = require('./scopedbrain')(res);
	...
	var indexRegex = indexPrefixStr === '' ? new RegExp('^(\\d+)') : RegExp('^' + indexPrefixStr + '?(\\d+)', 'i');

	var prefixIndex = function(idx) { return '' + indexPrefixStr + idx; };

	var unPrefixIndex = function(idx) {
		if (isNaN(idx)) {
			var m = idx.match(indexRegex);
			if (m !== null) {
	...
	var prefixIndiciesInPairs = function(pairs) {
	...
	var list = function() {
		var arr = brain.get(key);
		if (arr !== null && typeof arr === 'object' && arr.data) arr = arr.data;
		return (arr && Array.isArray(arr)) ? arr : [];
	};

	var search = function(items, idFunc) {
		var arr = list();
		var listIds = idFunc ? arr.map(idFunc) : arr;
		return items.map( function(item) {
			var idx = listIds.indexOf(idFunc ? idFunc(item) : item);
			return [item, (idx === -1)? idx : idx + 1 ];
		});
	};

	var isValidIndex = function( term ) { // is an integer, or a string of an integer or a string of a prefixed integer
...
module.exports.getTopicContext      = function(res) { return module.exports.getByType(res, 'topics'); };
