...
let env = require('./utils/env')(['CAPI_API_KEY']);
let rb = new (require('./utils/requestbuilder'))({
	global: {
		json: true,
		headers: {
	...
	byType: {
		getContent: {
			url: 'https://api.ft.com/content/items/v1/%s'
		}
	}
});
...
module.exports = {
	getStory: function(uuid) { // NB: this could return null if the capi request fails
		return rb.request('getContent', {
			...
			if(err) {
				let statusCode = err.statusCode || 'unknown status code';
				let errorMessage = (err.error && err.error.message)? err.error.message : 'unknown error.message';
